      ...
      <div className="flex w-full flex-col justify-between @3xl:flex-row">
        <div className="flex flex-col">
          {ListWrapper === "section" && (
            <H2
              id={paragraph.id}
              className={twMerge("mb-0 pb-0 font-medium", behaviors.list_paragraph?.heading_behavior === "hide" && "sr-only")}
            >
        ...
        {paragraph.suListButton?.url && (
          <Link
            ...
            className="mt-auto flex items-center gap-5 text-archway-dark"
          >
...
const getViewPagedItems = cache(async (viewId: string, displayId: string, contextualFilter?: Maybe<string[]>, pageSize?: Maybe<number>, page?: Maybe<number>, offset?: Maybe<number>): Promise<{items: NodeUnion[]; totalItems: number}> => {
  ...
  try {
    switch (`${viewId}--${displayId}`) {
      ...
      case "sup_books--book_list":
        filters = getViewFilters(["term_node_taxonomy_name_depth", "term_node_taxonomy_name_depth_2", "term_node_taxonomy_name_depth_1", "term_node_taxonomy_name_depth_3", "sup_book_work_id_number_value"], contextualFilter)
